...
def preprocessing(df_raw, data_folder) : 
    ...
    cancel_files_after_prediction(glob2.glob(os.path.join(outdir_1,'*.csv')))
    cancel_files_after_prediction(glob2.glob(os.path.join(outdir_2,'*.csv')))
    cancel_files_after_prediction(glob2.glob(os.path.join(outdir_3,'*.csv')))
    return list_all_df_with_operations_per_minute
